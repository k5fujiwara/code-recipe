...
## なぜ作ったのか — 「見に行く」をやめたかった

タスク管理に Todo アプリや Excel を試してみて引っかかったのは、**「自分から確認しに行く」手間** でした。そこで「その日のタスクや今後のタスクが自動で送られてきて、それをチェックするほうが効率的ではないか」と考えたのが、開発のきっかけです。

メールで自動的に届けば、タスクの見落としも、わざわざリストを開きに行く行動も減らせます。**「確認しに行く」という一手間をまるごと省く** ことをねらいました。

### なぜ Google Apps Script + スプレッドシートなのか

市販のタスク管理アプリではなく GAS とスプレッドシートを選んだのは、**会社の環境に素直に合うから** です。

- 会社で使っているのが Google アカウントで、会社のタスク管理として「会社のものとして」使える構成が GAS + スプレッドシートだった
- 会社のメールが法人用の Gmail なので、メール送信との連携が効率的だった
- 会社アカウントの中だけで完結するため、**セキュリティ面の心配がない**

外部サービスを増やさず、いま使っている環境の中で完結できることを重視しました。
...
## この仕様にした理由 — 毎日の運用で効くように

各仕様は、実際に自分で使うことを前提に「こうしたい」を形にしたものです。

- **重要度を色（★）で表示** — 優先度がひと目で分かるので、届いたその場で判断し、タスクを次々に片付けられる
- **出勤日に、受信トレイの上位へ届く時間帯に送信** — 見落としが起きないよう、送信時刻を調整している
- **チェックで完了シートへ自動移動** — 完了タスクがいつまでもリストに残らず、自分で削除する手間もいらない
- **完了が別シートに溜まる** — あとから「どれだけのタスクをこなしたか」を振り返れる。これが達成感につながり、自己肯定感も上がっていく

単に自動化するだけでなく、**続けたくなる仕組み** であることを大切にしました。
...
悩んだのは主に **トリガー設定** と **メールの形式** です。

トリガーは、指定した曜日・時間帯にちゃんと送信されるかを、**その時刻にならないと確認できない** のが厄介でした。動かして確かめては直す、という検証に時間がかかった部分です。

メール本文は、**見やすさと、GAS で表現できることの見極め** に時間を要しました。限られた表現の中で、優先度や期限が一目で伝わるレイアウトに落ち着かせるまで、試行錯誤を重ねています。
