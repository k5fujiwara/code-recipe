...
## なぜ作ったのか — Pythonでどこまでできるかを示す

DataLupe は、「プログラミングで何ができるか」を探りながら作った小さなアプリの一つです。大がかりな製品というより、**Python を使えば、データを読み込んで統計的に分析し、Webアプリとして公開するところまで一通りできる** ことを、実際に動く形で示すために開発しました。

データサイエンスや統計の入口を、コードだけでなく「触って試せるアプリ」として見せられれば、これから学ぶ人がイメージしやすくなる——そう考えたのがねらいです。
...
## 設計の考え方 — あえて AI を裏に置かない

DataLupe の日本語インサイトは、AI API を使わず、すべてルールベースで動いています。これは意図的な設計判断です。

裏に AI を組み込めば、インサイトの精度は上がり、オリジナリティも出せます。しかし DataLupe の目的は、**お金をかけずに動かせる開発事例** を示すことにあります。AI を前提にしなくても、Python と統計の考え方だけでここまで作れる——その「ベースとなる型」を、教育的な視点で紹介することを優先しました。

設計にあたっては AI に相談しながら方針を固めましたが、機能を盛り込むことより **シンプルさ・見やすさ・分かりやすさ** を最優先にしています。初心者が最初の一歩でつまずかないことを、何より大切にしました。
...
一番苦労したのは、**データを読み込んで「意図したとおりに表示する」** ことでした。数値の計算自体はできても、その結果から必要な部分を、見やすい形で画面に出すには一工夫も二工夫も必要で、表示まわりでは何度もエラーと向き合いました。分析結果は「正しく計算できること」と「分かりやすく見せられること」が別物だと、作りながら実感した部分です。
